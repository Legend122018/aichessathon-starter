"""Play two engines against each other across every core, and stop when the answer is clear.

Engine games are wall-clock timed, so running several at once only stays honest if each
one gets its own cores. Every worker is pinned, and each game gets a distinct opening so
the sample is not one line repeated.

Sequential testing (SPRT) stops as soon as the result is decided, which is usually far
sooner than a fixed game count and is what makes overnight tuning practical.

    python match.py --candidate candidate.py --champion agent.py --workers 7
"""
...
def worker(index: int, cores: list[int], candidate_path: str, champion_path: str,
           games: int, base_ms: int, inc_ms: int, seed: int) -> tuple[int, int, int]:
    pin(cores)
    candidate = load_agent(candidate_path, f"cand{index}")
    champion = load_agent(champion_path, f"champ{index}")
    rng = random.Random(seed)
    wins = draws = losses = 0
    for game in range(games):
        start = opening(rng)
        # Each opening is played once from each side, so colour cannot skew the result.
...
def main() -> None:
    ...
    parser.add_argument("--openings-per-batch", type=int, default=2,
                        help="openings per worker per round; each is played from both "
                             "sides, so games per round = workers x this x 2")
    ...
    while wins + draws + losses < args.max_games:
        ...
        for index in range(args.workers):
            first = (index * args.smt_stride) % total_cores
            cores = [(first + k) % total_cores for k in range(args.cores_per_game)]
            jobs.append((index, cores, args.candidate, args.champion,
                         args.openings_per_batch, args.base_ms, args.inc_ms,
                         args.seed * 100000 + batch * 1000 + index))
        with mp.Pool(args.workers) as pool:
            for w, d, losses_ in pool.starmap(worker, jobs):
                wins += w
                draws += d
                losses += losses_
    ...
    statistic = llr(wins, draws, losses, args.elo0, args.elo1)
    if statistic > 0:
        needed = int((wins + draws + losses) * upper / statistic)
        print(f"SPRT: inconclusive at the game limit. The trend is positive - about "
              f"{needed:,} games would settle it. Raise --max-games and rerun.")
    else:
