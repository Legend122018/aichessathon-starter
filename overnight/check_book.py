...
def main() -> None:
    ...
    print()
    check("book moves were played", hits > 0, f"{hits} moves from book")
    if hits:
        avg = book_ms / hits
        check("book moves are effectively free", avg < 50,
              f"{avg:.1f}ms average, vs {search_ms / max(searched, 1):.0f}ms searching")
        saved = hits * (search_ms / max(searched, 1))
        print(f"\n  {hits} book moves at {avg:.1f}ms each instead of "
              f"{search_ms / max(searched, 1):.0f}ms of search")
        print(f"  ~{saved / 1000:.1f}s of a 120s clock handed to the middlegame")
