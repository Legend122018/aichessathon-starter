...
# Trouble deserves time. A score this much worse than the previous iteration's means the
# move we were about to play may be the wrong one, so the soft budget is allowed to
# stretch by this factor while we look for a better one. The hard budget never moves: a
# flag loses the game outright and no amount of trouble is worth risking one.
TROUBLE_DROP = 50
TROUBLE_FACTOR = 1.5
...
def _python_get_move(fen: str, time_left_ms: int) -> str:
    ...
    try:
        ...
        for depth in range(1, MAX_PLY):
            ...
            previous_best = best_move
            # A score that just fell is the other case worth extra rope: the move we
            # were about to play may be refuted, so look for a better one. `hard` is
            # left alone, so this can never turn into a flag.
            trouble = depth > 1 and last_score - score >= TROUBLE_DROP
            last_score = score
            allowance = soft * (TROUBLE_FACTOR if trouble else 1.0)
            if elapsed_ms > allowance * (1.0 if unstable else 0.6):
                break
...
def _jit_choose(fen: str, time_left_ms: int) -> str:
    ...
    for depth in range(1, 64):
        ...
        previous = best
        trouble = have_last and last_score - score >= TROUBLE_DROP
        last_score = score
        have_last = True
        allowance = soft * (TROUBLE_FACTOR if trouble else 1.0)
        if elapsed > allowance * (1.0 if unstable else 0.6):
            break
